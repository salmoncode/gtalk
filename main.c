...
char *read_date;
char *read_time;
int n_speaker;	/* ��Ͽ���줿�üԿ� */
int spid;	/* ���ߤ��ü�ID */
FILE *logfp;
SPEAKER speaker[MAX_SPEAKER];
char *dic_file;
char *conf_audiodev = NULL;  /* �¹Ի��˻��ꤵ��륪���ǥ����ǥХ��� */
...
char input_text[MAX_TEXT_LEN];  /* ���Ϥ��줿�ƥ�����(�����Ĥ�) */
char spoken_text[MAX_TEXT_LEN]; /* �������Ϥ��줿ȯ�äΥƥ����� */
...
void sig_wait_da();
int getlinea( char *buf, int MAX_LENGTH );
/*******��for server mode ********/
...
#define MAX_COMMAND_LEN 8192  /* ���ϥ��ޥ�ɤκ���ʸ���� */
...
int read_command( char **args )
{
	...
	if( s_mode ) {
	        ...
	        if( getlinea( cline, MAX_COMMAND_LEN ) < 0)
		        setRun( "=", "EXIT" );
...
/*		parameter_generation();		*/
			...
			/* �����ǡ�prosBuf �Υǡ����ǲ��ǻ���Ĺ�����ꤹ�롣 */

			make_cumul_time(); /* ���ǻ���Ĺ�����Ѥ�׻� */
			modify_voice(); /* �üԤΥ����å������ѥ�᡼�����ѹ�(��������) */
...
void setParsedText( char *rel, char *filename )
{
	if( strcmp(rel,"=")==0 )  {
		refresh();
		text_analysis_file( filename );	/* �ƥ����Ȳ��� */
		parameter_generation();	/* �ѥ�᡼������(F0,MLSA�ե��륿����,��³Ĺ) */
		do_synthesis();		/* �����ȷ������� */
#ifdef PRINTDATA
		TmpMsg( "Synthesis Done.\n" );
